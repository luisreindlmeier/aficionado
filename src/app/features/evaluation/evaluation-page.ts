          ...
          >
            ...
            @if (f.score) {
              <div class="flex shrink-0 items-center gap-1.5">
                <span class="size-1.5 rounded-full" [style.background]="data.bandColor(f.score.band)"></span>
                <span class="text-[12px] tabular-nums text-foreground">{{ f.score.composite }}</span>
              </div>
            }
          </button>
        }
            ...
            <header class="flex items-start gap-4">
              ...
              <div class="min-w-0 flex-1">
                ...
                <p class="mt-1 text-[13px] leading-relaxed text-muted-foreground">
                  {{ f.headline }}@if (f.location) {, {{ f.location }}}
                </p>
                <div class="mt-3 flex flex-wrap gap-2">
                  @for (link of links(f); track link.label) {
                    ...
                    >
                      {{ link.label }}
                      <ng-icon name="heroArrowTopRightOnSquare" size="0.75rem" class="text-muted-foreground" />
                    </a>
                  }
                </div>
              </div>
                ...
                <div class="flex items-start gap-4">
                  ...
                  <div class="min-w-0 flex-1">
                    <div class="flex flex-wrap items-center gap-x-2 gap-y-1">
                      ...
                      <span class="rounded-full border-[0.5px] border-border px-2 py-0.5 text-[11px] text-muted-foreground">
                        {{ v.stage }}
                      </span>
                    </div>
                    <p class="mt-1 text-[13px] leading-relaxed text-muted-foreground">{{ v.tagline }}</p>
                    <p class="mt-2 text-[12px] text-muted-foreground">
                      {{ ventureMeta(v) }}
                    </p>
                  </div>
                  @if (v.website) {
                    <a [href]="'https://' + v.website" target="_blank" rel="noopener"
            ...
            @if (f.score; as s) {
              <section class="mt-4 rounded-xl border-[0.5px] border-border bg-card p-5">
                <div class="flex flex-wrap items-start justify-between gap-4">
                  <div class="min-w-0">
                    ...
                    <div class="flex items-baseline gap-2">
                      <span class="font-title text-[46px] leading-none tracking-[-0.02em] text-foreground">{{ s.composite }}</span>
                      <span class="text-[13px] text-muted-foreground">/ 100</span>
                    </div>
                    <p class="mt-2 text-[13px] text-muted-foreground">
                      {{ s.percentile }}th percentile vs the anchor set@if (s.anchorNeighbor) {, sits next to {{ s.anchorNeighbor }}}
                    </p>
                  </div>
                  <div class="flex shrink-0 flex-col items-end gap-2">
                    <span class="inline-flex items-center gap-1.5 rounded-full border-[0.5px] border-border px-2.5 py-1 text-[13px] font-medium text-foreground">
                      <span class="size-2 rounded-full" [style.background]="data.bandColor(s.band)"></span>
                      {{ s.band }}
                    </span>
                    <span class="inline-flex items-center gap-1.5 text-[12px] text-muted-foreground">
                      <span class="size-1.5 rounded-full" [style.background]="confColor(s.confidence)"></span>
                ...
                @if (routeToHuman(f)) {
                  <div class="mt-4 flex items-start gap-2 border-t-[0.5px] border-border pt-4 text-[12px]">
                    <ng-icon name="heroExclamationTriangle" size="0.9rem" class="mt-0.5 shrink-0 text-muted-foreground" />
                    <p class="text-muted-foreground">
                      Routed to a human. {{ humanReason(f) }}
                    </p>
                  </div>
                }
              ...
              <section class="mt-4 rounded-xl border-[0.5px] border-border bg-card p-5">
                <div class="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    ...
                    <p class="mt-0.5 text-[12px] text-muted-foreground">
                      Re-run the live pipeline against public sources and watch it think.
                    </p>
                  </div>
                  <button
                    type="button"
                  ...
                  >
                    <ng-icon [name]="running() ? 'heroArrowPath' : 'heroBolt'" size="0.9rem" [class.animate-spin]="running()" />
                ...
                @if (trace().length || running()) {
                  <div class="mt-4 flex flex-col gap-1.5 rounded-lg border-[0.5px] border-border bg-surface p-3 font-mono text-[11px] leading-relaxed">
                    @for (t of trace(); track $index) {
                      <div class="flex items-start gap-2">
                        ...
                        @if (t.detail) { <span class="text-muted-foreground">{{ t.detail }}</span> }
                      </div>
                    }
                    @if (running()) {
                ...
                @if (liveConnectors().length) {
                  <div class="mt-3 flex flex-wrap gap-1.5">
                    @for (c of liveConnectors(); track c.id) {
                      <span class="inline-flex items-center gap-1.5 rounded-full border-[0.5px] border-border px-2 py-0.5 text-[11px] text-muted-foreground">
              ...
              <div class="mt-8 mb-4 block">
                <h2 class="border-b-[0.5px] border-border pb-2.5 font-title text-[18px] leading-tight tracking-[-0.01em] text-foreground">
                ...
                @for (m of metricViews(f); track m.key) {
                  <article class="rounded-xl border-[0.5px] border-border bg-card p-5">
                    <div class="flex items-start gap-5">
                      ...
                      <div class="relative shrink-0">
                        <svg viewBox="0 0 100 100" class="size-24">
                          <circle cx="50" cy="50" r="42" fill="none" stroke="#e5e5e5" stroke-width="8" />
                          <circle cx="50" cy="50" r="42" fill="none" stroke-width="8" stroke-linecap="round"
                            transform="rotate(-90 50 50)" [attr.stroke]="m.color"
                            [attr.stroke-dasharray]="circ" [attr.stroke-dashoffset]="circ * (1 - m.score.score / 100)" />
                          <text x="50" y="50" text-anchor="middle" dominant-baseline="central" font-size="26" font-weight="600" fill="#111">{{ m.score.score }}</text>
                        </svg>
                      </div>
                      <!-- head + rationale -->
                      <div class="min-w-0 flex-1">
                        <div class="flex flex-wrap items-center gap-x-2 gap-y-1">
                          ...
                          <span class="rounded-full border-[0.5px] border-border px-2 py-0.5 text-[11px] text-muted-foreground">weight {{ pct(m.score.weight) }}%</span>
                          <span class="rounded-full border-[0.5px] border-border px-2 py-0.5 text-[11px] text-muted-foreground">{{ m.score.percentile }}th pct</span>
                          <span class="inline-flex items-center gap-1 text-[11px] text-muted-foreground">
                            <span class="size-1.5 rounded-full" [style.background]="confColor(m.score.confidence)"></span>
                            {{ m.score.confidence }} confidence
                          </span>
                        </div>
                        <p class="mt-1 text-[12px] text-muted-foreground">{{ m.caption }}</p>
                        <p class="mt-2 text-[13px] leading-relaxed text-foreground">{{ m.score.rationale }}</p>
                        @if (m.score.confidence === 'low') {
                          <div class="mt-2 inline-flex items-center gap-1.5 rounded-md border-[0.5px] border-border bg-surface px-2 py-1 text-[11px] text-muted-foreground">
                    ...
                    @if (m.score.receipts?.length) {
                      <ul class="mt-4 flex flex-col divide-y-[0.5px] divide-border border-t-[0.5px] border-border">
                        @for (r of m.score.receipts; track $index) {
                          <li class="flex items-start gap-3 py-3">
                            <span class="mt-0.5 shrink-0 rounded-full border-[0.5px] border-border px-2 py-0.5 text-[10px] text-muted-foreground">{{ r.connector }}</span>
                            <div class="min-w-0 flex-1">
                              <p class="text-[13px] text-foreground">{{ r.text }}</p>
                              @if (r.quote) { <p class="mt-0.5 text-[12px] italic text-muted-foreground">{{ r.quote }}</p> }
                            </div>
                            @if (r.url) {
                              <a [href]="normalizeUrl(r.url)" target="_blank" rel="noopener" class="mt-0.5 shrink-0 text-muted-foreground transition-colors hover:text-foreground" [title]="r.url">
              ...
              @if (f.fmf; as fmf) {
                <div class="mt-8 mb-4 block">
                  <h2 class="border-b-[0.5px] border-border pb-2.5 font-title text-[18px] leading-tight tracking-[-0.01em] text-foreground">Founder-market-fit</h2>
                </div>
                <section class="rounded-xl border-[0.5px] border-border bg-card p-5">
                  <div class="flex items-start gap-5">
                    <div class="shrink-0 text-center">
                      <div class="font-title text-[40px] leading-none" [style.color]="proofColor">{{ pctOf(fmf.similarity) }}</div>
                    ...
                    <div class="min-w-0 flex-1">
                      ...
                      @for (r of fmf.receipts; track $index) {
                        <a [href]="normalizeUrl(r.url)" target="_blank" rel="noopener"
                          class="mt-3 flex items-start gap-2 rounded-lg border-[0.5px] border-border bg-surface p-3 transition-colors hover:bg-accent">
                          <ng-icon name="heroLink" size="0.85rem" class="mt-0.5 shrink-0 text-muted-foreground" />
                          <span class="min-w-0 flex-1 text-[12px] text-foreground">{{ r.text }}</span>
                          <ng-icon name="heroArrowTopRightOnSquare" size="0.8rem" class="mt-0.5 shrink-0 text-muted-foreground" />
                        </a>
                      }
                    </div>
                  </div>
              ...
              @if (team(); as t) {
                <div class="mt-8 mb-4 block">
                  <h2 class="border-b-[0.5px] border-border pb-2.5 font-title text-[18px] leading-tight tracking-[-0.01em] text-foreground">Team complementarity</h2>
                </div>
                <section class="rounded-xl border-[0.5px] border-border bg-card p-5">
                    ...
                    <div>
                      ...
                      @for (axis of skillAxes; track axis.key) {
                        <div class="mb-2.5 flex items-center gap-3">
                          <span class="w-20 shrink-0 text-[12px] text-muted-foreground">{{ axis.label }}</span>
                          <div class="h-1.5 flex-1 overflow-hidden rounded-full bg-[#e5e5e5]">
                            <div class="h-full rounded-full bg-foreground" [style.width.%]="pctOf(t.coverage[axis.key])"></div>
                          </div>
                          <span class="w-8 shrink-0 text-right text-[11px] tabular-nums text-muted-foreground">{{ pctOf(t.coverage[axis.key]) }}</span>
                        </div>
                      }
                    </div>
                    <div class="text-[12px]">
                      @if (t.gaps.length) {
                        ...
                        <ul class="mb-3 flex flex-col gap-1">
                          @for (g of t.gaps; track g) {
                            <li class="flex items-start gap-2 text-foreground"><span class="mt-1.5 size-1.5 shrink-0 rounded-full bg-[#d97706]"></span>{{ g }}</li>
                          }
                        </ul>
                      }
                      @if (t.sharedHistory.length) {
                        ...
                        <ul class="flex flex-col gap-1">
                          @for (h of t.sharedHistory; track h) {
                            <li class="flex items-start gap-2 text-foreground"><span class="mt-1.5 size-1.5 shrink-0 rounded-full bg-[#16a34a]"></span>{{ h }}</li>
                          }
                        </ul>
                      }
                    </div>
                  ...
                  @if (t.bonus > 0) {
                    <p class="mt-4 border-t-[0.5px] border-border pt-3 text-[12px] text-muted-foreground">Team bonus, +{{ t.bonus }} for complementary coverage.</p>
                  }
                </section>
              }
              ...
              @if (f.trajectory.length) {
                <div class="mt-8 mb-4 block">
                  <h2 class="border-b-[0.5px] border-border pb-2.5 font-title text-[18px] leading-tight tracking-[-0.01em] text-foreground">Trajectory replay</h2>
                </div>
                <section class="rounded-xl border-[0.5px] border-border bg-card p-5">
                    ...
                    @for (p of f.trajectory; track $index; let last = $last) {
                      <div class="flex min-w-[120px] flex-1 flex-col">
                        <div class="flex items-center">
                          <span class="size-2.5 shrink-0 rounded-full border-[0.5px]" [style.borderColor]="trajColor" [style.background]="last ? trajColor : 'transparent'"></span>
                          @if (!last) { <span class="h-[0.5px] flex-1" [style.background]="'#e5e5e5'"></span> }
                        </div>
              ...
              <!-- 9. Red flags -->
              <div class="mt-8 mb-4 block">
                <h2 class="border-b-[0.5px] border-border pb-2.5 font-title text-[18px] leading-tight tracking-[-0.01em] text-foreground">Coherence and red flags</h2>
              </div>
                  ...
                  <div class="divide-y-[0.5px] divide-border">
                    @for (flag of f.redFlags; track flag.text) {
                      <div class="flex items-start gap-3 py-3.5">
                        <ng-icon name="heroExclamationTriangle" size="0.95rem" class="mt-0.5 shrink-0 text-muted-foreground" />
                        <div class="min-w-0 flex-1">
                          <p class="text-[13px] text-foreground">{{ flag.text }}</p>
                          <p class="mt-0.5 text-[12px] text-muted-foreground">{{ flag.note }}</p>
                        </div>
                        <span class="inline-flex shrink-0 items-center gap-1.5 text-[11px] capitalize text-muted-foreground">
                          <span class="size-1.5 rounded-full" [style.background]="sevColor(flag.severity)"></span>
                          {{ flag.severity }}
                        </span>
                      </div>
                    }
              ...
              <p class="mt-6 text-[13px] text-muted-foreground">This founder is queued for evaluation.</p>
            }
          </div>
        }
      </div>
...
export class EvaluationPage {
  ...
  protected metricViews(f: Founder): MetricView[] {
    ...
    const s = f.score;
    return [
      { key: 'Proof' as Metric, color: METRIC_COLORS.Proof, score: s.proof, caption: CAPTIONS.Proof },
      { key: 'Gravity' as Metric, color: METRIC_COLORS.Gravity, score: s.gravity, caption: CAPTIONS.Gravity },
      { key: 'Trajectory' as Metric, color: METRIC_COLORS.Trajectory, score: s.trajectory, caption: CAPTIONS.Trajectory },
    ].sort((a, b) => b.score.weight - a.score.weight);
  }

  protected links(f: Founder): { label: string; url: string }[] {
    const out: { label: string; url: string }[] = [];
    if (f.handles.github) out.push({ label: 'GitHub', url: `https://github.com/${f.handles.github}` });
    ...
    if (f.handles.website) out.push({ label: f.handles.website, url: `https://${f.handles.website}` });
    return out;
  }
  ...
  protected routeToHuman(f: Founder): boolean {
    return f.score?.confidence === 'low' || (f.score ? f.score.confidence !== 'high' && !!f.note : false);
  }

  protected humanReason(f: Founder): string {
    return f.note ?? 'Evidence is thin on at least one dimension; a partner should confirm before deciding.';
  }
  ...
  private apply(ev: EvalEvent): void {
    switch (ev.type) {
      ...
      case 'phase':
        this.trace.update((t) => [
          ...t,
          { at: now(), label: `Fan-out: ${ev.metric}`, detail: ev.connectors.join(', '), kind: 'fetch', metric: ev.metric },
        ]);
        break;
      case 'metric':
        this.trace.update((t) => [
          ...t,
          { at: now(), label: `${ev.score.metric} scored ${ev.score.score}`, detail: `${ev.score.confidence} confidence`, kind: 'reduce', metric: ev.score.metric },
        ]);
        break;
      case 'final':
        this.trace.update((t) => [
          ...t,
          { at: now(), label: `Composite ${ev.score.composite}, ${ev.score.band}`, kind: 'done' },
        ]);
        break;
      case 'error':
